"""
Day 4 - Task 4: Workflow Automation API (THIS FILE IS ONLY MEANT FOR N8N TESTING, NOT THE FINAL WORLFOW)
...
async def monitoring_middleware(request: Request, call_next):
    ...
    try:
        ...
        monitoring.record_api_request(request.url.path, request.method, response.status_code, (time.perf_counter()-started)*1000)
        return response
    except Exception:
        monitoring.record_api_request(request.url.path, request.method, 500, (time.perf_counter()-started)*1000)
        raise
...
def readiness():
    detail=health_check()
    status=200 if detail.get("healthy") else 503
    return JSONResponse(status_code=status, content=detail)

@app.get("/metrics/summary")
def metrics_summary(window_minutes: int = 60):
    return monitoring.get_summary(window_minutes=max(1,min(window_minutes,1440)))
...
@app.get("/health")
def health_check():
    """Day 6 Task 5: a real health check, not just 'the process is up' -
    verifies the actual dependencies each endpoint needs (DB file exists
    and is queryable, required LLM env vars are set) rather than always
    returning 200 regardless of whether the service can do anything
    useful. Returns 200 with a per-check breakdown either way - a
    monitoring/orchestration tool should read the "healthy" field, not
    just the status code, same "explicit status, not just HTTP code"
    convention this file already uses everywhere else (see module
    docstring)."""
    checks = {}

    try:
        import structured_retrieval
        structured_retrieval.get_distinct_cities()
        checks["database"] = {"ok": True}
    except Exception as e:
        checks["database"] = {"ok": False, "error": str(e)}

    import llm_client
    checks["primary_llm_configured"] = {"ok": bool(llm_client.BASE_URL and llm_client.API_KEY)}
    checks["groq_configured"] = {"ok": bool(llm_client.GROQ_API_KEY)}
    checks["gemini_configured"] = {"ok": bool(llm_client.GEMINI_API_KEY)}
    checks["at_least_one_llm_fallback_available"] = {
        "ok": bool(llm_client.GROQ_API_KEY or llm_client.GEMINI_API_KEY)
    }

    import calendar_integration as _cal
    import email_automation as _mail
    cal_creds_path = getattr(_cal, "GOOGLE_CREDENTIALS_PATH", None)
    checks["calendar_configured"] = {
        "ok": bool(cal_creds_path and os.path.exists(cal_creds_path)),
        "detail": f"GOOGLE_CREDENTIALS_PATH={cal_creds_path!r}",
    }
    mail_creds_path = getattr(_mail, "GOOGLE_CREDENTIALS_PATH", None)
    checks["email_configured"] = {
        "ok": bool(mail_creds_path and os.path.exists(mail_creds_path)),
        "detail": f"GOOGLE_CREDENTIALS_PATH={mail_creds_path!r}",
    }

    checks["deepgram_configured"] = {"ok": bool(os.getenv("DEEPGRAM_API_KEY"))}
    checks["fish_audio_configured"] = {"ok": bool(os.getenv("FISH_AUDIO_API_KEY"))}

    require_calendar = os.getenv("REQUIRE_CALENDAR_FOR_READINESS", "1") == "1"
    require_email = os.getenv("REQUIRE_EMAIL_FOR_READINESS", "1") == "1"
    require_voice = os.getenv("REQUIRE_VOICE_FOR_READINESS", "0") == "1"
    llm_ok = bool(
        checks["primary_llm_configured"]["ok"]
        or checks["groq_configured"]["ok"]
        or checks["gemini_configured"]["ok"]
    )
    healthy = bool(checks["database"]["ok"] and llm_ok)
    if require_calendar:
        healthy = healthy and checks["calendar_configured"]["ok"]
    if require_email:
        healthy = healthy and checks["email_configured"]["ok"]
    if require_voice:
        healthy = healthy and checks["deepgram_configured"]["ok"] and checks["fish_audio_configured"]["ok"]

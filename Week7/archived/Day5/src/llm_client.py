...
generate_reply_stream() is the Day 3 Task 1 latency path ("Speech -> LLM
-> Voice, under 2 seconds") - used by tests/test_streaming_voice_pipeline.py,
not by the LangGraph nodes (graph.run_turn() needs the reply as one
complete string to route/validate on, so nodes.py stays on generate_reply()).
"""
...
# Every real LLM response (primary or Gemini fallback) gets its token usage
# appended here; graph_logger.py's traced_node drains this around each
# node's execution to attribute usage to the node that caused it, without
# generate_reply()/generate_with_tools() needing new return values threaded
# through their ~4 call sites in nodes.py.
...
def drain_usage() -> List[Dict[str, Any]]:
    """Pops and returns every usage record accumulated since the last drain."""
    global _usage_log
    drained, _usage_log = _usage_log, []
    return drained
...
def _record_openai_usage(resp) -> None:
    ...
    if usage is None:
        return
    record_usage(PRIMARY_MODEL, usage.prompt_tokens or 0, usage.completion_tokens or 0, usage.total_tokens or 0)
...
def generate_reply_stream(system_prompt: Optional[str], user_prompt: str) -> Iterator[Tuple[str, int]]:
    """Streaming generation for the Day 3 Task 1 latency path: yields
    (sentence_text, elapsed_ms) as each complete sentence appears in the
    streamed response, elapsed_ms measured from this call's start - so the
    FIRST yielded item's elapsed_ms is "time to first sentence," the number
    that actually determines when TTS/voice can start, not total generation
    time. No Gemini fallback here (unlike generate_reply()/generate_with_tools()):
    this is the latency-critical path, and duplicating streaming support for
    a second provider isn't worth it for a test script - callers needing a
    guaranteed reply should use generate_reply() instead."""
    client = _get_primary_client()
    messages = []
    if system_prompt:
        messages.append({"role": "system", "content": system_prompt})

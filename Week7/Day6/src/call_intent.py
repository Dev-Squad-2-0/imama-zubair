...
_NEGATION_MARKERS = (
    "nahi", "nahin", "nhi", "not", "no",
    "نہیں", "نہ",
)


def _contains_non_negated_keyword(text: str, keyword: str, window: int = 18) -> bool:
    """Return True when *keyword* occurs at least once outside a nearby
    negation window. This keeps deterministic routing but prevents phrases
    such as ``warehouse nahi``, ``not commercial``, or ``رینٹ نہیں`` from
    being interpreted as positive intent signals.

    The window is deliberately small: it only answers whether this one
    keyword is locally negated; it is not a general sentiment/parser layer.
    """
    start = 0
    while True:
        ...
        after = text[idx + len(keyword):idx + len(keyword) + window].strip()
        locally_negated = any(
...
def _contains_any_positive(text: str, keywords) -> bool:
    return any(_contains_non_negated_keyword(text, kw) for kw in keywords)
...
_BUYER_KEYWORDS = [
    "buy", "buying", "purchase", "khareed", "kharid", "khareedna", "kharidna",
    "خریدنا", "خریدیں", "خرید",
]


def has_explicit_buyer_signal(customer_text: str) -> bool:
    """True only when this turn positively says purchase/buy.

    ``buyer_inquiry`` is also the classifier's safe fallback, so callers that
    need to distinguish an explicit switch to buying from a no-signal turn
    should use this helper instead of treating every buyer result as evidence.
    """
    return _contains_any_positive(customer_text.lower(), _BUYER_KEYWORDS)
...
def classify_call_intent(customer_text: str) -> str:
    ...
    if _contains_any_positive(lowered, _SELLER_KEYWORDS):
        return "seller_inquiry"

    if _contains_any_positive(lowered, _INVESTMENT_KEYWORDS):
        return "investment_inquiry"
    if _contains_any_positive(lowered, _COMMERCIAL_KEYWORDS):
        return "commercial_inquiry"
    if _contains_any_positive(lowered, _RENTAL_KEYWORDS):

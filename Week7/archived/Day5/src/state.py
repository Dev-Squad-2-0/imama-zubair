...
class AgentState(TypedDict, total=False):
    ...
    greeted: bool                                       # has greeting_node already fired for this session?
    pending_appointment_datetime: Optional[str]         # ISO datetime the customer already gave, remembered
                                                          # across turns where the current turn doesn't restate
                                                          # it (e.g. a phone-confirmation "haan ji" reply)
...
def new_agent_state(session_id: str) -> AgentState:
    return AgentState(
        ...
        user_profile={"client_name": None, "client_phone": None,
                       "client_phone_pending": None, "client_phone_confirmed": False},
...
def slots_from_text(existing_profile: Dict[str, Any], existing_prefs: Dict[str, Any],
    ...
    memory.slots.client_phone_pending = existing_profile.get("client_phone_pending")
    memory.slots.client_phone_confirmed = existing_profile.get("client_phone_confirmed", False)
    ...
    return {
        "user_profile": {
            ...
            "client_phone_pending": memory.slots.client_phone_pending,
            "client_phone_confirmed": memory.slots.client_phone_confirmed,
        },
        "property_preferences": {
            **existing_prefs,

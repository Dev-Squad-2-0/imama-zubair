...
defining its own client/property/time fields, since Task 1 and Task 2 need
the exact same information (system_prompt.md: "After booking, send an
email confirmation and log the appointment in the CRM" — same event, two
side effects). Keeping one shared details object means conversation_agent.py
builds it once and passes it to both create_appointment_event() and
send_appointment_notification().

Auth: Gmail API, OAuth installed-app flow, same GOOGLE_CREDENTIALS_PATH
Desktop app client as calendar_integration.py. Gmail's send scope is
narrower than Calendar's, so this keeps its own token cache file rather
than sharing one with calendar_integration.py — the two modules stay
independently usable (e.g. you could grant Gmail access later without
touching the calendar token, or vice versa).

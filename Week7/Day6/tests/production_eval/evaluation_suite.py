...
APPOINTMENT = [
    {
        "id":"appt_01","category":"appointment","description":"complete one-turn booking against an exact real property",
        "turns":["Mera naam Sara hai. DHA Phase 6 Lahore mein apartment property number 33 ki appointment kal 6 baje book kar dein."],
        "checks":[
            ("booking reaches booking node", lambda s,t,r: "booking" in t[-1]),
            ("booking succeeds", lambda s,t,r: (s.get("appointment_status") or {}).get("status") == "booked"),
        ],
    },
    {
        "id":"appt_02","category":"appointment","description":"progressive slot filling across separate turns",
        "turns":["Mujhe appointment book karni hai.","Mera naam Bilal hai.","DHA Phase 6.","Apartment.","Property number 33.","Kal 4 baje."],
        "checks":[
            ("appointment intent stays in booking flow", lambda s,t,r: sum("booking" in x for x in t) >= 4),
            ("booking succeeds after progressive slot filling", lambda s,t,r: (s.get("appointment_status") or {}).get("status") == "booked"),
        ],
    },
    {
        "id":"appt_03","category":"appointment","description":"Urdu booking with exact listing selection",
        "turns":["میرا نام حسن ہے۔ ڈی ایچ اے فیز 6 لاہور میں اپارٹمنٹ چاہیے۔", "پراپرٹی نمبر 33 کی اپوائنٹمنٹ کل شام پانچ بجے بک کر دیں۔"],
        "checks":[("booking succeeds", lambda s,t,r: (s.get("appointment_status") or {}).get("status") == "booked")],
    },
    {
        "id":"appt_04","category":"appointment","description":"missing booking info asks one clarification instead of guessing",
        "turns":["اپوائنٹمنٹ بک کرنی ہے۔"],
        "checks":[("asks for missing info", lambda s,t,r: s.get("clarification_needed") is True and bool(s.get("missing_fields")))],
    },
    {
        "id":"appt_05","category":"appointment","description":"caller provides multiple slots together and agent saves all available information",
        "turns":["Mera naam Adeel hai, F-10 Islamabad mein house chahiye aur kal 7 baje appointment book karni hai.","Property number 34."],
        "checks":[("booking succeeds once exact property is selected", lambda s,t,r: (s.get("appointment_status") or {}).get("status") == "booked")],
    },
]
...
CANCELLATION = [
    {
        "id":"cancel_01","category":"cancellation","description":"book then cancel same event, native/Roman mix",
        "turns":["Mera naam Ayesha hai. DHA Phase 6 Lahore mein apartment property number 33 ki appointment kal 5 baje book kar dein.","میری اپوائنٹمنٹ کینسل کر دیں۔"],
        "checks":[("cancellation reached", lambda s,t,r: "cancellation" in t[-1]),("status cancelled", lambda s,t,r:(s.get("appointment_status") or {}).get("status")=="cancelled")],
    },
    {
        "id":"cancel_02","category":"cancellation","description":"book then cancel using English loanword",
        "turns":["Mera naam Usman hai. F-10 Islamabad mein house property number 34 ki appointment kal 3 baje book kar dein.","Appointment cancel kar dein please."],
        "checks":[("status cancelled", lambda s,t,r:(s.get("appointment_status") or {}).get("status")=="cancelled")],
    },
    {
        "id":"cancel_03","category":"cancellation","description":"cancel with no prior appointment fails safely",
        "turns":["میری اپوائنٹمنٹ کینسل کر دیں۔"],
        "checks":[("does not fake cancellation", lambda s,t,r: not s.get("appointment_status")),("gives a reply", lambda s,t,r: bool(r[-1].strip()))],
    },
]
...
RESCHEDULING = [
    {
        "id":"resched_01","category":"rescheduling","description":"book then reschedule with natural Urdu date/time",
        "turns":["Mera naam Zain hai. DHA Phase 6 Lahore mein apartment property number 33 ki appointment kal 5 baje book kar dein.","میں ریس کیجول کرنا چاہتا ہوں۔","پرسوں دن 12 بجے۔"],
        "checks":[("sticky reschedule reaches node", lambda s,t,r: "rescheduling" in t[-1]),("status rescheduled", lambda s,t,r:(s.get("appointment_status") or {}).get("status")=="rescheduled")],
    },
    {
        "id":"resched_02","category":"rescheduling","description":"book then reschedule with dual-date sentence",
        "turns":["Mera naam Hina hai. F-10 Islamabad mein house property number 34 ki appointment kal 2 baje book kar dein.","Meri kal 2 baje ki appointment ko parso 4 baje reschedule kar dein."],
        "checks":[("status rescheduled", lambda s,t,r:(s.get("appointment_status") or {}).get("status")=="rescheduled")],
    },
    {
        "id":"resched_03","category":"rescheduling","description":"reschedule with no prior appointment fails safely",
        "turns":["ریس کیجول کر دیں۔"],
        "checks":[("no fake appointment", lambda s,t,r:not s.get("appointment_status")),("gives a reply", lambda s,t,r:bool(r[-1].strip()))],
    },
]

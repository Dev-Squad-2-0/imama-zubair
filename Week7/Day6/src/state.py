...
class AgentState(TypedDict, total=False):
    ...
    caller_id: Optional[str]                            # phone number from the TELEPHONY layer itself (e.g.
                                                          ...
                                                          # graph.run_turn(..., caller_id=...). The live mic CLI can
                                                          # receive a simulated caller id via --caller-id; a real
                                                          # telephony adapter should pass its From/caller field here.
...
def slots_from_text(existing_profile: Dict[str, Any], existing_prefs: Dict[str, Any],
                     existing_decline_count: int, customer_text: str,
    ...
    # Recommendation memory must be copied into the throwaway ConversationMemory
    # too. Without this, "Us se sasti koi option hai?" cannot see the last shown
    # price and therefore cannot lower the budget ceiling.
    memory.slots.last_shown_property_ids = list(
        existing_prefs.get("last_shown_property_ids") or []
    )
    memory.slots.last_shown_min_price = existing_prefs.get("last_shown_min_price")
    memory.slots.last_shown_max_price = existing_prefs.get("last_shown_max_price")

    memory.slots.decline_count = existing_decline_count

    expected_fields = expected_fields or []
    memory.update_from_customer_text(
        customer_text,
        expect_name=("client_name" in expected_fields),
    )

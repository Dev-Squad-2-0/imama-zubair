...
def _route_after_intent(state: AgentState) -> str:
    ...
    # No explicit book/reschedule/cancel phrase THIS turn - but if the
    # previous turn ended mid a booking/reschedule/cancel clarification loop
    # (asked for a missing name/phone/property/date, or a phone read-back
    # confirmation - see nodes.py's booking_node) and this turn doesn't look
    # like a new topic, keep routing to that same node instead of falling
    # through to recommendation/rag. Without this, a bare "Ahmed" or "haan
    # sahi hai" answering the agent's own question has no booking keyword of
    # its own and would otherwise get silently misrouted, stalling the
    # booking instead of completing it. tool_outputs.last_write_action
    # persists across turns (nodes.py's _write_action_update always sets it)
    # until the next write-action node overwrites it, so this only kicks in
    # while a write action is genuinely still unresolved.
    last_action = state["tool_outputs"].get("last_write_action")
    if last_action and not last_action.get("success") and not _looks_like_factual_question(state["customer_text"]):
